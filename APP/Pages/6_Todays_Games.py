...
@st.cache_data
def load_data():
    # Load schedule (predictor input) and daily predictor (historical results)
    schedule_path = "Data/26_March_Madness_Databook/Schedule_Transfer.csv"
...
def train_model(daily_df):
    # Filter rows where we have results
    train_df = daily_df.dropna(subset=["Points", "Opp Points"]).copy()

    # Drop obvious non-numeric or text columns
    ...
    # Train-test split (though we retrain fully each time)
...
st.dataframe(
    predict_df[["Team", "Opponent", "Predicted Points", "Points", "Opp Points"]].sort_values("Predicted Points", ascending=False),
...
for _, row in predict_df.iterrows():
    ...
    # Fix the tuple creation that caused TypeError
